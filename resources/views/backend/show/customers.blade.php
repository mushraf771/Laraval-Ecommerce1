                ...
                <tbody>
                    @foreach ($customers as $customer)
                        ...
                        <tr
                            ...
                            {{-- <td class="px-6 py-4">
                                <img src="{{ Storage::url($customer->logo) }}" alt="" class=" w-14 h-16">
                            </td> --}}

                            <td class="px-6 py-4">
                                <div class="flex gap-x-4">
                                    @if ($customer->status == 1)
                                        <a href="{{ url('dashboard/customer/status/0') }}/{{ $customer->id }}"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Hide</a>
                                    @else
                                    <a href="{{ url('dashboard/customer/status/1') }}/{{ $customer->id }}"
                                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Show</a>

                                    @endif
                                    <a href="{{ url('dashboard/customer/show') }}/{{ $customer->id }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">show</a>
                                    {{-- <form method="POST" onsubmit="return confirm(' Are You Sure...?');"
                                    action={{ route('dashboard.brand.destroy', $customer->id) }}>
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="font-medium text-red-600 dark:text-red-500 hover:underline">Delete</button>
                                    </form> --}}
                                    {{-- <a href="{{ url('dashboard/brand/delete') }}/{{ $customer->id }}"
