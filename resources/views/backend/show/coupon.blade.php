...
@section('content')
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-6">
        ...
        <div class="flex items-center justify-between pb-4">
            ...
            <div class="">
                <a href="{{ route('dashboard.coupon.create') }}" class="p-1 bg-primary text-white">
                    {{ __('Add Coupon') }}
                </a>
            </div>
        </div>
            ...
            <tbody>
                @foreach ($coupons as $coupons)
                    ...
                    <tr
                        ...
                        <td class="px-6 py-4">
                            {{-- <img src="{{ Storage::url($coupons->image) }}" alt="" class=" w-14 h-16"> --}}
                        </td>
                        <td class="px-6 py-4">
                            {{-- {{ $coupons->description }} --}}
                        </td>
                        ...
                        <div class="flex gap-x-4">
                            ...
                            @if ($coupons->status == 1)
                            <a href="{{ url('dashboard/coupon/status/0') }}/{{ $coupons->id }}"
                                class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Hide</a>
                        @else
                        <a href="{{ url('dashboard/coupon/status/1') }}/{{ $coupons->id }}"
                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Show</a>

                        @endif
                            <a href="{{ url('dashboard/coupon/edit') }}/{{ $coupons->id }}"
                                class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                            {{-- <form method="POST" onsubmit="return confirm(' Are You Sure...?');"
                                action={{ route('dashboard.coupons.destroy', $coupons->id) }}>
                                @csrf
                                @method('DELETE')
                                <button type="submit"
...
<a href="{{ url('dashboard/coupon/delete')}}/{{$coupons->id  }}" class=""><button type="button"
                                    class="font-medium text-red-600 dark:text-red-500 hover:underline">Delete</button>
</a>                        </div>
                    </td>
                    </tr>
    ...
    <div class="px-6 py-4 w-full">
        <h2 class="text-3xl">No coupons Added Yet </h2>
    </div>
